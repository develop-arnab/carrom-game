...
public class GameManager : NetworkBehaviour {
    ...
    //             if (IsServer) {
    //         // Register for client connect events so the server can start the game when all players join
    ...
    public override void OnNetworkSpawn() {
        ...
        if (IsServer) {
            ...
            // If the clients already connected by the time this object spawns (lobby -> scene flow),
            // start the game immediately
            Debug.Log("OnNetworkSpawn: Checking connected clients count: " + NetworkManager.Singleton.ConnectedClientsList.Count);
            if (NetworkManager.Singleton.ConnectedClientsList.Count >= 2)
            {
                Debug.Log("Both players already connected on server at OnNetworkSpawn; starting game.");
    ...
    {
        // Clean up server callbacks when this object is despawned
    ...
    private void TriggerOnGameStartedRpc() {
        Debug.Log("TriggerOnGameStartedRpc");
