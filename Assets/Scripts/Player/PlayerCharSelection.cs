...
public class PlayerCharSelection : NetworkBehaviour
{
    ...
    private void Start()
    {
        ...
        // NEW: If this instance is the owner (local player) and we had a pre-selection
        // from the non-networked main menu, send it to the server so the networked
        // CharacterSelection scene reflects the player's chosen character.
        // For the host (IsServer true and IsOwner true) we can directly set the value.
/*        if (NonNetworkSelectionData.SelectedCharacter >= 0 && NonNetworkSelectionData.SelectedCharacter < CharacterSelectionManager.Instance.charactersData.Length)
        {
            int preSelected = NonNetworkSelectionData.SelectedCharacter;

            if (IsOwner)
            {
                // If running as host and this object runs on server as owner, set directly
                if (IsServer)
                {
                    m_charSelected.Value = preSelected;
                }
                else
                {
                    // If client-owner, request server to set the value
                    ChangeCharacterSelectionServerRpc(preSelected);
                }
            }
        }*/
    }
